// Translateable an adapted version of Better in-place-editing
// http://github.com/nakajima/nakatype/wikis/better-edit-in-place-translateable-js

var Translateable = Class.create({
    ...
    parseField: function() {
				parsed_id = this.element.id.replace(/ /g, '_');
        var matches = parsed_id.match(/(.*)_\[(.*)\]_(.*)/);
        
        if(matches == null) {
					alert(parsed_id + " could not be found");
				}
    ...
    // Create the editing form for the translateable and inserts it after the element.
    ...
    // Sets up event handles for translateable.
    ...
    save: function(event) {
        ...
        new Ajax.Request(url + ".json", {
            ...
            onSuccess: function(transport) {
                ...
                if (Translateable.afterSave) {
                    Translateable.afterSave(this);
                }
                this.cancel();
            }.bind(this),
    ...
    // Event handler that restores original translateable value and hides form.
    ...
    // Removes translateable behavior from an element.
    clobber: function() {
...
// Translateable class methods.
Object.extend(Translateable, {
    options: {
        saveText: 'Save',
        cancelText: 'Cancel'
    },
    create: function(element) {
        new Translateable(element);
    },

    setupAll: function(klass) {
        klass = klass || '.translateable';
        $$(klass).each(Translateable.create);
    },
		is_active: function() {
			return Translateable.translateKeyPressed;
		},
		translateKeyCode: 224,
		translateKeyPressed: false,
		keyHandlerDown: function(event) {
			var key = event.which || event.keyCode;
			if(key == Translateable.translateKeyCode) Translateable.translateKeyPressed = true;
		},
		keyHandlerUp: function(event) {
			var key = event.which || event.keyCode;
			if(key == Translateable.translateKeyCode) Translateable.translateKeyPressed = false;
		}
});

// Helper method for event delegation
Element.addMethods({
    translateable: function(element, options) {
        new Translateable(element, options).edit();
    }
});

Event.observe(window, 'load', function() {
	Event.observe(document, 'keydown', Translateable.keyHandlerDown, false);
	Event.observe(document, 'keyup', Translateable.keyHandlerUp, false);
}, false);
